...
export default function NewTripPage() {
  ...
  const allStarts = segments
    .map((s) => s.startDate)
    .filter(Boolean)
    .sort()
  const allEnds = segments
    .map((s) => s.endDate)
    .filter(Boolean)
    .sort()
  ...
  return (
    <>
      {tripLimitSheet && (
        <UpgradeSheet required="pro" feature="tripLimit" onClose={() => setTripLimitSheet(false)} />
      )}
      {multiDestUpgrade && (
        <UpgradeSheet
          required="pro"
          feature="multiDestination"
            ...
            {segments.map((seg, i) => (
              <div key={i} className="space-y-2">
                ...
                >
                  ...
                  <span
                    className={`text-[15px] ${seg.startDate ? 'text-ink font-medium' : 'text-ink3'}`}
                  >
