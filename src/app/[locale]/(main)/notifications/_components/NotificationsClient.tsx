...
function TripInviteIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
      <circle cx="8" cy="6" r="3.5" stroke="var(--color-primary)" strokeWidth="1.4" />
      <path
        d="M2 17c0-3 2.7-5 6-5"
...
function InfoIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
      <circle cx="10" cy="10" r="7" stroke="var(--color-ink3)" strokeWidth="1.4" />
      <path
...
function getConfig(type: string): IconConfig {
  return TYPE_CONFIG[type] ?? { bg: 'var(--color-bg2)', icon: <InfoIcon /> }
}
...
export default function NotificationsClient({
  ...
  return (
    <div className="bg-bg2 min-h-dvh">
      <div className="border-border flex h-[54px] items-center justify-between border-b bg-white px-5">
        <span className="text-ink text-[22px] font-bold tracking-[-0.4px]">{t('title')}</span>
        {unreadCount > 0 && (
          <button onClick={handleMarkAll} className="text-primary text-[13px] font-medium">
            {t('markAllRead')}
          </button>
        )}
      </div>
      ...
      {notifList.length === 0 && (
        <div className="flex flex-col items-center justify-center gap-3 py-32">
          <div className="bg-bg2 flex h-16 w-16 items-center justify-center rounded-[20px]">
          ...
          <p className="text-ink text-[15px] font-semibold">{t('emptyTitle')}</p>
          <p className="text-ink3 text-[13px]">{t('emptyDesc')}</p>
        </div>
      )}
...
function NotifGroup({
  ...
  return (
    <div className="px-4 pt-3">
      <p className="text-ink3 mb-2 text-[11px] font-semibold tracking-[1px] uppercase">{label}</p>
  ...
  return (
    <div
      ...
      style={{
        ...
        borderColor: n.is_read ? 'var(--color-border)' : '#C5DBFF',
        cursor: tripId ? 'pointer' : 'default',
      }}
      onClick={handleCardClick}
    >
      {!n.is_read && <div className="bg-primary absolute top-3.5 right-3.5 h-2 w-2 rounded-full" />}
      ...
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1 pr-4">
          <p className="text-ink text-[13px] leading-[1.4] font-semibold">{getTitle()}</p>
          {getDesc() && <p className="text-ink2 mt-0.5 text-[12px] leading-[1.5]">{getDesc()}</p>}
          <p className="text-ink3 mt-1.5 text-[11px]">{timeAgo(n.created_at, locale)}</p>
          ...
          {showInviteButtons && (
            <div className="mt-2.5 flex gap-1.5">
              <button
                ...
                className="bg-primary flex flex-1 items-center justify-center rounded-[8px] py-2 text-[12px] font-semibold text-white disabled:opacity-60"
              >
                {pending === 'accept' ? (
              ...
              <button
                ...
                className="border-border bg-bg2 text-ink2 flex flex-1 items-center justify-center rounded-[8px] border py-2 text-[12px] font-semibold disabled:opacity-60"
              >
                {pending === 'decline' ? (
                  <div className="border-ink3 h-3.5 w-3.5 animate-spin rounded-full border-2 border-t-transparent" />
                ) : (
                  t('decline')
                )}
              </button>
