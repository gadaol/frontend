...
function getPlanLabel(subscription: Subscription | null, t: (k: string) => string): string {
  if (!subscription || subscription.plan === 'free') return t('subscription.planLabel.free')
          ...
          <MenuItem
            ...
            sub={
              plan === 'free'
                ...
                : t('currentPlanOf', { plan: PLAN_LABEL[plan] })
            }
              ...
              {(['gada', 'rog'] as CharacterId[]).map((id) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      <span
                        className={`mb-1 inline-block rounded-full px-2 py-0.5 text-[10px] font-semibold ${active ? 'bg-primary/10 text-primary' : 'text-ink3 bg-gray-100'}`}
                      >
                        {info.type[locale as 'ko' | 'en']}
                      </span>
                      <p
                        className={`text-[14px] font-bold ${active ? 'text-primary' : 'text-ink'}`}
                      >
        ...
        */}
        ...
        <div className="border-border mx-4 mt-3 mb-6 overflow-hidden rounded-2xl border bg-white">
          ...
          >
            ...
            <span className="text-error flex-1 text-left text-[14px] font-medium">
              {t('logout')}
            </span>
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
              <path
